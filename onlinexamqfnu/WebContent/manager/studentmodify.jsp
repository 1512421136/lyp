  ...
  <form action="<%=path %>/studentModifyServlet" method="post" name="form1" id="form1">
    <table width="900" border="0" align="center" class="word_darkGrey">
      ...
      <tr>
        ...
        <c:choose>
        <c:when test="${deptName ==student.deptName}">
         <option value='${deptName}' selected="selected">${deptName }</option>
        </c:when>
        <c:otherwise>
      ...
      <tr>
        ...
        <c:choose>
        <c:when test="${classId == student.classId }">
          <option value='${c.id }'  selected="selected">${classId} }</option>
          </c:when>
          <c:otherwise>
